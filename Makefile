...
S3_BUCKET="tfstate-$(PRODUCT)-$(ENV)-$(REGION)"
DYNAMODB_TABLE="tflock-$(PRODUCT)-$(ENV)-$(REGION)"
...
set-env:
	...
	@if [ -z $(PRODUCT) ]; then \
		echo "$(BOLD)$(RED)PRODUCT was not set$(RESET)"; \
		ERROR=1; \
	 fi
...
apply-ci: prep ## Have terraform do the things. This will cost money.
	@terraform apply \
		-lock=true \
		-input=false \
		-refresh=true \
...
destroy-ci: prep ## Destroy the things
	@terraform destroy \
		-lock=true \
		-input=false \
		-refresh=true \
